...
export async function GET(req: NextRequest, res: NextResponse) {
    ...
    try {
        ...
        return new NextResponse(JSON.stringify({ message: 'Transcript downloaded successfully' }), {
            status: 200,
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        return new NextResponse(JSON.stringify({ message: 'Failed to download transcript' }), {
            status: 500,
            headers: {
                'Content-Type': 'application/json',
            },
